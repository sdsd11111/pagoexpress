...
export default function BancosClient() {
    ...
    return (
        <main className="min-h-screen bg-white text-pe-black selection:bg-pe-yellow/30">
            ...
            <section className="relative h-auto lg:h-[70vh] lg:min-h-0 flex flex-col justify-center bg-pe-black overflow-hidden">
                <div className="max-w-[1600px] mx-auto px-4 lg:px-12 w-full h-full">
                    <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center h-full py-12 lg:py-0">
                        <motion.div initial="hidden" animate="visible" variants={fadeUp} className="relative z-10 text-left order-2 lg:order-1">
                            ...
                            <h1 className="text-2xl sm:text-4xl lg:text-5xl font-black leading-[1.05] tracking-tight mb-8 uppercase italic text-white">
                            ...
                            <div className="flex flex-col sm:flex-row items-center gap-4 mb-16">
                                <Link href="#directorio" className="w-full sm:w-auto px-10 py-5 bg-pe-yellow text-pe-black font-black rounded-2xl hover:scale-105 active:scale-95 transition-all uppercase tracking-widest text-[11px]">Directorio Bancario</Link>
                                <Link href="#servicios-bancarios" className="w-full sm:w-auto px-10 py-5 border border-white/10 text-white font-black rounded-2xl hover:bg-white/5 transition-all uppercase tracking-widest text-[11px] flex items-center justify-center gap-2 backdrop-blur-sm group">Servicios <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" /></Link>
                            </div>
                            <div className="flex items-center gap-3">
                                {heroSlides.map((slide, i) => (
                                    <button key={i} onClick={() => setCurrentHeroSlide(i)} className={`transition-all duration-700 rounded-full ${currentHeroSlide === i ? "w-12 h-1.5 bg-pe-yellow" : "w-3 h-1.5 bg-white/10 hover:bg-white/20"}`} aria-label={slide.label} />
                                ))}
                            </div>
                        </motion.div>
                        <div className="relative h-[350px] sm:h-[450px] lg:h-[550px] order-1 lg:order-2">
                            ...
                            <div className="relative w-full h-full rounded-[4rem] overflow-hidden border border-white/10 shadow-3xl group p-4 lg:p-6">
                                <AnimatePresence initial={false} mode="wait">
                                    <motion.div key={currentHeroSlide} initial={{ x: 300, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -300, opacity: 0 }} transition={{ type: "spring", stiffness: 300, damping: 30 }} className="absolute inset-4 lg:inset-6 rounded-[2.5rem] overflow-hidden">
                                        <Image src={heroSlides[currentHeroSlide].image} alt={heroSlides[currentHeroSlide].label} fill className="object-cover transition-transform duration-1000 group-hover:scale-110" priority />
                                ...
                                <div className="absolute top-1/2 -translate-y-1/2 left-6 right-6 flex justify-between opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => setCurrentHeroSlide(prev => (prev === 0 ? heroSlides.length - 1 : prev - 1))} className="w-12 h-12 rounded-full bg-pe-black/50 backdrop-blur-md border border-white/10 flex items-center justify-center text-white hover:bg-pe-yellow hover:text-pe-black transition-all"><ArrowRight className="w-5 h-5 rotate-180" /></button>
                                    <button onClick={() => setCurrentHeroSlide(prev => (prev === heroSlides.length - 1 ? 0 : prev + 1))} className="w-12 h-12 rounded-full bg-pe-black/50 backdrop-blur-md border border-white/10 flex items-center justify-center text-white hover:bg-pe-yellow hover:text-pe-black transition-all"><ArrowRight className="w-5 h-5" /></button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <section id="servicios-bancarios" className="py-24 bg-white border-b border-pe-gray-100">
                <div className="max-w-7xl mx-auto px-4">
                    <div className="text-center mb-16">
                        ...
                        <h2 className="text-3xl md:text-4xl font-black mb-4">Servicios Bancarios <span className="text-pe-yellow-dark">Unificados</span></h2>
                    ...
                    <div className="grid md:grid-cols-3 gap-8">
                        {[
                            ...
                            <motion.div key={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="p-8 rounded-[3rem] bg-pe-gray-50 border border-pe-gray-200 hover:border-pe-yellow/30 transition-all group hover:shadow-xl">
                ...
                <div className="max-w-7xl mx-auto px-4">
                    <div className="flex flex-col md:flex-row items-end justify-between mb-16 gap-6">
                        <div className="max-w-xl">
                            <h2 className="text-3xl md:text-4xl font-black mb-4">Directorio de Instituciones</h2>
                        ...
                        <div className="relative w-full md:w-80">
                            ...
                            <input type="text" placeholder="Buscar banco o coop..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-white border border-pe-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-pe-yellow/50" />
                        </div>
                    </div>

                    <div className="hidden sm:grid grid-cols-2 lg:grid-cols-4 gap-6">
                        {filteredEntities.map((entity, i) => (
                            <motion.div key={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} onClick={() => setSelectedEntity(entity)} className={`group p-8 rounded-[2.5rem] bg-white border ${entity.border} transition-all hover:scale-[1.03] shadow-sm hover:shadow-xl cursor-pointer`}>
                                <div className={`w-12 h-12 rounded-xl ${entity.color} ${entity.text} flex items-center justify-center mb-6`}><Landmark className="w-6 h-6" /></div>
                                ...
                                <div className="space-y-3">
                                    {entity.services.map((s, idx) => (
                                        <div key={idx} className="flex items-center gap-2 text-xs font-bold text-pe-black/70"><Check className={`w-4 h-4 ${entity.text}`} />{s}</div>
                                    ))}
                                </div>
                            </motion.div>
                        ))}
                            ...
                            {Array.from({ length: Math.ceil(filteredEntities.length / 3) }).map((_, slideIndex) => (
                                <div key={slideIndex} className="min-w-[85vw] snap-center flex flex-col gap-4">
                                    {filteredEntities.slice(slideIndex * 3, slideIndex * 3 + 3).map((entity, i) => (
                                        <div key={i} onClick={() => setSelectedEntity(entity)} className={`p-6 rounded-[2rem] bg-white border ${entity.border} shadow-sm cursor-pointer active:scale-[0.98] transition-transform`}>
                                            <div className="flex items-center gap-4 mb-4">
                                                <div className={`w-10 h-10 rounded-xl ${entity.color} ${entity.text} flex items-center justify-center`}><Landmark className="w-5 h-5" /></div>
                                                <div>
                                            ...
                                            <div className="flex flex-wrap gap-2 text-[10px] font-bold text-pe-black/70">
                                                {entity.services.map((s, idx) => (
                                                    <div key={idx} className="flex items-center gap-1 bg-pe-gray-50 px-2 py-1 rounded-lg"><Check className={`w-3 h-3 ${entity.text}`} />{s}</div>
                                                ))}
                                            </div>
                                        </div>
                                    ))}
            ...
            <section className="py-24 bg-pe-black text-white relative overflow-hidden">
                <div className="max-w-7xl mx-auto px-4 relative z-10">
                    <h2 className="text-3xl font-black mb-12 text-center">Transferencias <span className="text-pe-yellow">Directas PagoExpress</span></h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {featuredBanks.map((bank, i) => (
                            <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} onClick={() => setSelectedBank(bank)} className="relative h-64 rounded-[3rem] overflow-hidden group cursor-pointer border border-white/10">
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-pe-black p-8 flex flex-col justify-end">
                                    ...
                                    <div className="flex items-center gap-2 text-pe-yellow font-bold text-[10px] uppercase tracking-widest">Ver datos <ArrowRight className="w-4 h-4 group-hover:translate-x-2 transition-transform" /></div>
                                </div>
                            </motion.div>
                        ))}
                    </div>
                ...
                <div className="max-w-[1600px] mx-auto px-4">
                    <div className="bg-white rounded-[4rem] overflow-hidden shadow-3xl flex flex-col lg:flex-row border border-pe-gray-100 min-h-[750px]">
                        <div className="lg:w-1/3 bg-pe-black p-10 lg:p-16 flex flex-col justify-center text-white border-r border-white/5">
                            <div className="inline-flex items-center gap-2 text-pe-yellow mb-8"><Clock className="w-5 h-5" /><span className="font-black uppercase tracking-[0.2em] text-[10px]">Diferenciador Clave</span></div>
                            <h3 className="text-4xl lg:text-5xl font-black mb-10 leading-[1.1]">Atendemos cuando el banco <span className="text-pe-yellow">ya cerró.</span></h3>
                        ...
                        <div className="lg:w-1/3 bg-pe-gray-50 p-10 flex items-center justify-center border-r border-pe-gray-100">
                            <div className="w-full h-full max-h-[600px] rounded-[3rem] overflow-hidden shadow-2xl border border-pe-gray-200">
                                <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3980.103773883154!2d-79.1999948!3d-3.999083700000001!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x91cb397d349953f7%3A0xdea692caf196a0e7!2sPago%20Express%20Servicios%20Financieros!5e0!3m2!1ses-419!2sec!4v1772581023091!5m2!1ses-419!2sec" className="w-full h-full border-0 grayscale hover:grayscale-0 transition-all duration-700" allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" />
                            </div>
                        </div>
                        <div className="lg:w-1/3 bg-white p-10 lg:p-16 flex flex-col justify-center">
                            ...
                            <h3 className="text-3xl font-black mb-10 text-pe-black uppercase leading-none">Visítanos <br /> en <span className="text-pe-black/30">nuestras agencias</span></h3>
                            <div className="space-y-4 mb-8">
                                {[
                                    ...
                                    <div key={i} className="p-5 rounded-3xl bg-pe-gray-50 border border-pe-gray-100 group hover:border-pe-yellow transition-all">
                                        <div className="flex items-center gap-4">
                                            <div className="w-10 h-10 rounded-xl bg-pe-black flex items-center justify-center shrink-0"><MapPin className="w-5 h-5 text-pe-yellow" /></div>
                                            <div>
                            ...
                            <Link href="https://maps.google.com" target="_blank" className="w-full py-4 bg-pe-black text-white rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-3 hover:translate-y-[-2px] transition-all shadow-xl"><MapPin className="w-4 h-4 text-pe-yellow" /> ABRIR EN GOOGLE MAPS</Link>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section className="py-24 text-center">
                ...
                <div className="flex justify-center gap-4">
                    <Link href="https://wa.me/593" className="w-14 h-14 bg-green-500 rounded-full flex items-center justify-center text-white shadow-xl hover:scale-110 transition-transform"><MessageCircle className="w-6 h-6" /></Link>
                    <Link href="#guia" className="w-14 h-14 bg-pe-black rounded-full flex items-center justify-center text-white shadow-xl hover:scale-110 transition-transform"><HelpCircle className="w-6 h-6" /></Link>
                </div>
            </section>
        </main>
    );
